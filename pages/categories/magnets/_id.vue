...
.main
  .breadcrumbs
    .back
      nuxt-link(to="/categories/magnets")
...
export default {
      ...
      benefits: [
          {
            ...
            text: `Особое внимание к производству. \n Каждый магнит мы обрабатываем вручную, чтобы мелкие детали были максимально четкими.`
          },
          {
  ...
  async fetch ({ store, params }) {
        await store.dispatch('getMagnet', params.id)
  },
  head(){
        return {
  ...
  computed: {
    getItem() {
      return this.$store.state.magnet
    },
    oldPrice() {
      return this.$store.state.magnet.filter(item => item.id === +this.$route.params.id)[0].acf.price * 1.3
    },
    product() {
      return {
        title: this.getItem.title.rendered,
