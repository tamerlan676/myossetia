...
export default {
  ...
  computed: {
    getItem() {
      return this.$store.state.cuples.filter(item => item.id === +this.$route.params.id)[0]
    },
    oldPrice() {
      return this.$store.state.cuples.filter(item => item.id === +this.$route.params.id)[0].acf.price * 1.3
    },
    product() {
      return {
        title: this.getItem.title.rendered,
