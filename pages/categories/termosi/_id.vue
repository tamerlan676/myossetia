...
.main
  .breadcrumbs
    .back
      nuxt-link(to="/categories/termosi")
...
export default {
  ...
  async fetch ({ store }) {
        await store.dispatch('getTermoses')
  },
  head(){
        return {
  ...
  computed: {
    getItem() {
      return this.$store.state.termosi.filter(item => item.id === +this.$route.params.id)[0]
    },
    oldPrice() {
      return this.$store.state.termosi.filter(item => item.id === +this.$route.params.id)[0].acf.price * 1.3
    },
    product() {
      return {
        title: this.getItem.title.rendered,
