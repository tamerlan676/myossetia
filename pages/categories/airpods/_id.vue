...
.main
  .breadcrumbs
    .back
      nuxt-link(to="/categories/airpods")
...
export default {
  ...
  async fetch ({ store }) {
        await store.dispatch('getAirPods')
  },
  head(){
        return {
  ...
  computed: {
    getItem() {
      return this.$store.state.airpods.filter(item => item.id === +this.$route.params.id)[0]
    },
    oldPrice() {
      return this.$store.state.airpods.filter(item => item.id === +this.$route.params.id)[0].acf.price * 1.3
    },
    getModels() {
      return this.$store.state.airpodsModels
    },
    product() {
      return {
        title: this.getItem.title.rendered,
