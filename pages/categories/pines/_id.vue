...
export default {
  ...
  async fetch ({ store }) {
        await store.dispatch('getPines')
  },
  computed: {
    getItem() {
      return this.$store.state.pines.filter(item => item.id === +this.$route.params.id)[0]
    },
    oldPrice() {
      return this.$store.state.pines.filter(item => item.id === +this.$route.params.id)[0].acf.price * 1.3
    },
    product() {
      return {
